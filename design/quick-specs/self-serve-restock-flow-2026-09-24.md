...
6. 꼬치·고수도 진열대 품목이다 (상점 → 창고 → 진열대, 해금 없음, 대체재로 쓰이지 않음). `SKEWER_CHANCE` 확률로 꼬치 1–2개(종류 무작위, 진열대·계산대·가격 모두 꼬치 1개 단위, 꼬치 1개 = 1,000원)를, `CILANTRO_CHANCE` 확률로 고수 한 줌을 진열대에서 가져간다. 진열대에 없으면 못 담고 해당 품목이 `missing`에 들어가 투덜(`grumble`)한다. 고수는 시들고(perishable) 꼬치는 시들지 않는다. 시작 창고: 꼬치 각 10, 고수 20 (시들어서 하루 약 3박스 필요).
...
### G. 플레이어 피드백 (플레이테스트 2026-09-25 후속)
1. 신선도: 부패 품목 칸의 가장 오래된 묶음이 `WILT_WARN_RATIO` 이하만 남으면 스프라이트가 누렇게 변하고 "🥀 곧 시듦" 태그가 붙는다. 묶음이 시들어 버려지면 그 칸이 `WILT_FLASH_SEC` 동안 번쩍인다.
2. 계산 결과: 선결제 즉시 결과 토스트(`FEEDBACK_TOAST_SEC` 유지). 정확하면 칭찬, 틀리면 차액 + 원인 + 정답 구성(저울/고기/꼬치/고수). 원인 우선순위: 주문표 조리 방식 불일치 → 숨은 재료 미발견 → 추가금 오입력.
3. 과다 청구는 손님 컴플레인으로 평판 `RATING_DELTA.overcharge` (브리프의 "과다 청구 → 컴플레인·평판↓").
...
| `WILT_WARN_RATIO` | 0.3 | 0.2–0.5 | feel | "곧 시듦" 경고 시작점 (남은 신선도 비율) |
| `WILT_FLASH_SEC` | 1.5 | 1–3 | feel | 시든 칸 번쩍임 시간 |
| `FEEDBACK_TOAST_SEC` | 5 | 3–8 | feel | 계산 결과 토스트 유지 시간 (기본 2.5초보다 길게) |
| `RATING_DELTA.overcharge` | −0.1 | −0.2–0 | curve | 과다 청구 컴플레인 |
