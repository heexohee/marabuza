...
6. 꼬치·고수도 진열대 품목이다 (상점 → 창고 → 진열대, 해금 없음, 대체재로 쓰이지 않음). `SKEWER_CHANCE` 확률로 꼬치 1–2개(종류 무작위, 진열대 단위 = 꼬치 1개)를, `CILANTRO_CHANCE` 확률로 고수 한 줌을 진열대에서 가져간다. 진열대에 없으면 못 담고 해당 품목이 `missing`에 들어가 투덜(`grumble`)한다. 새우 꼬치는 새우 2마리 = 꼬치 1개. 고수는 시들고(perishable) 꼬치는 시들지 않는다. 시작 창고: 꼬치 각 10, 고수 20 (시들어서 하루 약 3박스 필요).
